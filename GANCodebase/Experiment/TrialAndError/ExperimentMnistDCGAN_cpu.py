...
# Save model
saveGeneratorModelPath = "./Model/TrainedModel/DCGAN/Generator" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".pt"
saveDiscriminatorModelPath = "./Model/TrainedModel/DCGAN/Discriminator" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".pt"
...
for k in range(5*5):
    ...
    j = k % 5
    ax[i, j].cla()
    ax[i, j].imshow(G(normalDTrainDs[k]).detach().view(28, 28).numpy(), cmap='Greys')
saveFigPath = "./Figure/Experiment/TrialAndError/ExperimentMnistDCGAN/GeneratedImage_5x5" + "_dim" \
...
saveFigPath = "./Figure/Experiment/TrialAndError/ExperimentMnistDCGAN/EpochLossPlot" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".png"
CreateFile(saveFigPath)
plt.savefig(saveFigPath) # Save viz
plt.show()
