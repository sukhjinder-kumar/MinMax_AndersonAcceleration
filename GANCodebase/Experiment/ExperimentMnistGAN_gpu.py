...
for k in range(5*5):
    ...
    j = k % 5
    ax[i, j].cla()
    ax[i, j].imshow(G(normal100DimDTrainDs[k].to(mpsDevice)).cpu().detach().view(28, 28).numpy(), cmap='Greys')
saveFigPath = "./Figure/Experiment/ExperimentMnistGAN/GeneratedImage_5x5" + "_dim" \
...
saveFigPath = "./Figure/Experiment/ExperimentMnistGAN/EpochLossPlot" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".png"
plt.savefig(saveFigPath) # Save viz
plt.show()
