...
def TrainModel(mnistDl,normalDDl,normalGDl,G,D,numEpochs):
    opt = AltGDA([
        {'params':D.parameters(), 'lr':0.0001},
        {'params':G.parameters(), 'lr':0.0003}])
    ...
    for epoch in range(numEpochs):
        ...
        for i, (mnistBatch,normalDBatch,normalGBatch) in enumerate(zip(mnistDl,normalDDl,normalGDl)):
            realData = mnistBatch[0].view(-1,28*28).to(mpsDevice)
...
# Save model
saveGeneratorModelPath = "./Model/TrainedModel/MnistGAN/AltGDA/Generator" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".pt"
saveDiscriminatorModelPath = "./Model/TrainedModel/MnistGAN/AltGDA/Discriminator" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".pt"
...
for k in range(5*5):
    ...
    j = k % 5
    ax[i, j].cla()
    ax[i, j].imshow(G(normalDTrainDs[k].to(mpsDevice)).cpu().detach().view(28, 28).numpy(), cmap='Greys')
saveFigPath = "./Figure/Experiment/MNIST/MnistGAN_AltGDA/GeneratedImage_5x5" + "_dim" \
...
saveFigPath = "./Figure/Experiment/MNIST/MnistGAN_AltGDA/EpochLossPlot" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".png"
CreateFile(saveFigPath)
plt.savefig(saveFigPath) # Save viz
plt.show()
