# Boilerplate for each experiment file.
# To allow for module calling and messy relative path issues
...
from Algorithm.ConOpt import ConOpt 
...
def TrainModel(mnistDl, normalDDl, normalGDl, G, D, numEpochs):
    opt = ConOpt(G, D, lr=1e-3, gamma=0.1)
...
# Train Model
numEpochs = 1
epochs, gLosses, dLosses = TrainModel(mnistTrainDl,normalDTrainDl,normalGTrainDl,G,D,numEpochs=numEpochs)
endTime = time.time()
...
# Save model
saveGeneratorModelPath = "./Model/TrainedModel/MnistGAN/ConOpt/Generator" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".pt"
saveDiscriminatorModelPath = "./Model/TrainedModel/MnistGAN/ConOpt/Discriminator" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".pt"
...
for k in range(5*5):
    ...
    j = k % 5
    ax[i, j].cla()
    ax[i, j].imshow(G(normalDTrainDs[k]).view(28, 28).detach().numpy(), cmap='Greys')
saveFigPath = "./Figure/Experiment/MNIST/MnistGAN_ConOpt/GeneratedImage_5x5" + "_dim" \
...
saveFigPath = "./Figure/Experiment/MNIST/MnistGAN_ConOpt/EpochLossPlot" + "_dim" \
    + str(dim) + "_batchSize" + str(batchSize) + "_numEpochs" + str(numEpochs) + ".png"
CreateFile(saveFigPath)
plt.savefig(saveFigPath) # Save viz
plt.show()
